...
template<typename T>
void RansEncInit(RansState<T>* r)
{
    *r = RANS32_L;
};
...
RansState<T> RansEncRenorm(RansState<T> x, P** pptr, uint32_t freq, uint32_t scale_bits)
{
...
{
    // renormalize
    RansState<T> x = RansEncRenorm(*r, pptr, freq, scale_bits);
...
{
	RansDecAdvance<T,P>(r, pptr, sym->start, sym->freq, scale_bits);
};
...
{
	RansDecAdvanceStep<T>(r, sym->start, sym->freq, scale_bits);
};
